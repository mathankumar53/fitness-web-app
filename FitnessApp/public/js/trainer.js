    ...
    this.trainees.forEach(client => {
      ...
      if (currentWeight && client.targetWeightKg) {
        ...
        if (client.fitnessGoal === 'weight_loss' && diff <= 0) {
          ...
          progressBadge = `<span class="streak-badge" style="background: rgba(245,158,11,0.1); color: var(--accent-orange);"><i class="fa-solid fa-arrow-down"></i> ${diff}kg to go</span>`;
        }
      }
      ...
      html += `
        <div class="glass-card" style="display: flex; flex-direction: column; gap: 12px; padding: 20px;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 8px; font-size: 13px; background: rgba(0,0,0,0.2); padding: 12px; border-radius: 8px;">
            ...
            <div><span style="color:var(--text-muted)">Streak:</span> <span style="color:var(--text-main); font-weight: 500;"><i class="fa-solid fa-fire" style="color:var(--accent-orange)"></i> ${client.streakDays || 0}</span></div>
          </div>
